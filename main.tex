...
A propriedade anti-Ramsey ${G\rbarrow H}$ é monótona em grafos aleatórios, logo, admite uma função limiar \cite{BoTh87}, que nós denotamos por $\hat{p}_H$. 


Antes de discutirmos os resultados conhecidos da propriedade anti-Ramsey, precisamos considerar diferentes densidades de um grafo. 
...
No entanto, em \cite{KoKoMo16+}, o mesmo grupo de pesquisadores obteve uma resposta negativa para essa pergunta, eles exibiram uma família infinita de grafos cuja função limiar é inferior a $n^{-1/m_2(H)}$.
...
\end{teorema}
    ...
    O resultado sobre ciclos do Teorema \ref{teo:cyclecomplete} segue do seguinte lema:
...
Observam que tal sequência impõe certas configurações restritas de como os novos ciclos aparecem entre os subgrafos, assim trabalhando na combinação dessas configurações detalham alguns casos de como colorir arestas dessa sequência de subgrafos para evitar $C_\ell$ rainbow.
     Parte das ideias desse artigo são retomadas nas demonstrações descritas na próxima seção.
...
\section{Limiar para grafos bipartidos completos}
    ...
    Nesta seção começamos a discussão que prova para grafos bipartidos completos um resultado semelhante ao que foi comentado anteriormente sobre ciclos e grafos completos, a saber, a propriedade anti-Ramsey de grafos bipartidos em grafos aleatórios.
...
\label{sec:rbestrut}
     ...
     Para isto, precisamos analisar as interseções de arestas das cópias de grafos bipartidos completos, dado que colorir tais arestas de maneira a evitar $\K$ rainbow pode exigir uma coloração engenhosa. 
    ...
    Da mesma forma, seja $H_2:= G[a,b]$, onde $a = A_1\cap
    A_2$ e $b = B_1\cap B_2$.  
    Uma vez que 
    $v(H_1 \cup H_2) = (\ell + r) +
    ...
    Faremos frequente aplicação, direta ou indiretamente, da Proposição~\ref{lemma:inter_a} em nossas provas.
     ...
     Vamos seguir uma estratégia próxima à encontrada em \cite{barros2021anti}, da propriedade anti-Ramsey de ciclos, que analisou interseções de arestas entre ciclos no grafo alvo de coloração e com densidade limitada.
    ...
    Ou seja, existe um $K \cong \K$ em $H_{i}$ tal que $K \not\subset H_{i-1}$ e $E(K) \cap E(H_{i-1}) \neq\varnothing$.
     ...
     Será suficiente fornecer uma coloração para uma $\K$-componente arbitrária de $G$. %%%%EXPLICAR?
 ...
 A próxima proposição lida com o caso em que $A_1\cup A_2$ intersecta a $B_1\cup B_2$, sua prova segue a mesma ideia da Proposition \ref{lemma:inter_a} com um argumento de densidade similar à desigualdade \ref{eq:pairs-ab}, porém apresenta uma solução única para esta, o que força uma configuração específica de intersecções de arestas.
    ...
    \end{proposicao}
     \begin{proof}
         ...
         É evidente que $a \geq 1$, $b \geq 1$, e também que $a + b+y = r$ e $a + (\ell-x) \leq \ell$ são condições da construção. 
     ...
     % Logo, seria 
     é que tal configuração é densa o suficiente para ser uma $\K$-componente isolada do grafo $G$, pois não permite intersecções com mais grafos bipartidos completos. Com isso, obter uma coloração própria desta componente de modo a evitar cópias rainbow de $\K$ é trivial e podemos nos voltar para outras interseções.
     
     Nós podemos nos focar na intersecção entre duas cópias de $\K$ em $G$ que é da forma $K_{a,b}$ para alguns inteiros $a$ e $b$ (Proposição \ref{lemma:inter_a}). 
	 O grafo $G$ pode conter uma intersecção de múltiplas cópias de
    $\K$, dado que este caso seria um subgrafo de $K_{\ell+x,r+y}$ onde $x$ ou $y$ são inteiros positivos, e, como veremos, esta intersecção múltipla pode afetar a maneira como colorimos a $\K$-componente de $G$. 
...
\begin{proof}
        ...
        Suponha que existe um $s$-ciclo no grafo de interseção $\mathcal{I}(G)$ -- o que representa uma cadeia de $\K$'s em $G$ --  considerando só interseções da forma $K_{1,1}$.
       ...
       Tratar os múltiplos $\K$ que podem ser gerados por intersecções como um único vértice de $\mathcal{I}(G)$ torna tal grafo acíclico pelo mesmo argumento.
\end{proof}
 ...
 \begin{lema}\label{lema:config}
    ...
    \end{lema}
\begin{proof}
        Vamos provar o resultado para todos valores possíveis de $v_i$, i.e., $0\leq v_i \leq \ell+r-2$ juntamente do número de novos $\K$ adicionados a $H_i$.
   ...
   tem interseção com algum $H_j$ com $j<i-1$, o que contradiz o Fato \ref{fato:ciclos}.
    ...
    Esta configuração forma um único $\K$ com $H_i$,  agora consideramos casos onde mais de um $\K$ aparece em $H_i$.
