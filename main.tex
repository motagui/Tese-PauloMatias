...
%%% existe simetria entre l e r aqui? provavel que nao importe
 ...
 \end{figure}
    ...
    $\K$, dado que este caso seria um subgrafo de $K_{\ell+x,r+y}$ onde $x$ ou $y$ são inteiros positivos, e, como veremos, esta intersecção múltipla pode afetar a maneira como colorimos a $\K$-componente de $G$. %%%% explicar
...
\end{proof}
  ...
  Para um conjunto de vértices $X = \{x_1, \ldots, x_\ell\}$, dados inteiros $1 \leq a \leq b \leq \ell$, denotamos por $X_a^b$ o subconjunto $\{x_a, \ldots, x_b\}$ de $X$. 
    ...
    As configurações de $\K$-componentes isoladas que são descritas pela Proposição \ref{aff:casoP1} serão denominadas do \textit{tipo C}.
 ...
 \begin{lema}\label{lema:config}
    ...
    \end{lema}
\begin{proof}
        O caso mais simples é o bipartido $K$ formar uma configuração do tipo C que é uma $\K$-componente isolada.
        
        Vamos provar o resultado sobre os tipos A e B para todos valores possíveis de $v_i$, i.e., $0\leq v_i \leq \ell+r-2$ juntamente do número de novos $\K$ adicionados a $H_i$.
...
configuração~\hyperref[configB]{$(B_{2,0})$}.
 ...
 \medskip \textbf{Caso 3.} 
	...
	tal quantidade $e_i$ é insuficiente para inverter dois vértices da partição no subgrafo que resulte em outro $\K$.
