...
\begin{center}
    \vspace*{2cm}
    \textbf{\Large{Padrões de colorações de arestas e propriedades anti-Ramsey }}\\
    ...
    financeiro da FAPESP(Proc.2018/22768-1)}    
    \vskip 0.35cm
...
\thispagestyle{empty}
    ...
    \begin{itemize}
		...
		\item Prof. Dr. Fábio Happ Botler - UFRJ %[sem ponto final]
		\item Prof. Dr. Luiz Freire Moreira - UFPE %[sem ponto final]
         \item Prof. Dr. Roberto Freitas Parente - UFBA %[sem ponto final]
        \item Prof. Dr. Walner Mendonça dos Santos - USP %[sem ponto final]
    \end{itemize}
...
Essa quantidade de cores era desconhecida para $n$ par e conseguimos computá-la para alguns casos pequenos e também para uma família infinita de valores de $n$.
...
Provamos que o valor da função limiar para $G(n,p) \rbarrow K_{\ell,r}$ é dado por $n^{(\ell+r-2)/(\ell r-1)}$, para grafos bipartidos completos $\K$ com $\ell,r \geq 3$. 
Observamos também que o limiar é assintoticamente menor que este valor se $\ell=2$ e $r\geq 2$.
\\
...
We prove that the value of the threshold function for $G(n,p) \rbarrow K_{\ell,r}$ is given by $n^{(\ell+r-2)/(\ell r-1)}$ , for complete bipartite graphs $\K$ with $\ell,r \geq 3$. We also observe that the threshold is asymptotically smaller than this value if $\ell=2$ and $r\geq 2$.
\\
...
A Teoria de Ramsey é a área da matemática que investiga a existência de ordem de subconjuntos dentro de conjuntos grandes de objetos. 
O nome desse campo se deve ao resultado clássico de Frank P. Ramsey \cite{Ra} que, em 1930, enquanto estudava um problema de lógica proposicional, provou o conhecido ``Teorema de Ramsey'' no qual afirma que para quaisquer inteiros positivos $t$ e $q$, existe um inteiro $n$ tal que toda $q$-coloração das arestas do grafo completo $K_n$ contém uma cópia monocromática de $K_t$.
...
Com isso, chama-se de \emph{número de Ramsey de ordem} $q$, denotado por $r_q(H_1,H_2, \ldots ,H_q)$,
 o menor inteiro positivo $N$ tal que existe uma partição de arestas de $K_N$ em $q$ classes de cores distintas, fornecendo $q$ subgrafos $G_1, G_2, \ldots,G_q$, e,
pelo menos um dos grafos $G_i$ contém um subgrafo isomorfo a $H_i$, para $1 \leq i \leq q$.
...
Avanços relevantes foram obtidos recentemente, Campos, Griffiths, Morris e Sahasrabudhe~\cite{campos2023exponential}
obtiveram a primeira melhora exponencial no número de Ramsey $r(K_t)$ desde Erd\H{o}s e Szekeres~\cite{EsZ} em 1935;
enquanto em outra frente, Mattheus e Verstraete~\cite{mattheus2023asymptotics} resolvem uma conjectura de Erd\H{o}s~\cite{chung98:_erd} quanto ao limite inferior sobre o número de Ramsey $r(K_4, K_t)$.
...
%% A princípio, Erd\H{o}s \cite{erdos81:_i} perguntou se o número size-Ramsey de caminhos seria tal que $\hat{r}(P_n) = \omega(n)$ e $\hat{r}(P_n) = o(n^2)$. Em resposta, Beck \cite{Be83} forneceu uma construção probabilística de que o número size-Ramsey de caminhos é linear, i.e., $\hat{r}(P_n) = \Theta(n)$.  
...
Aqui, uma função limiar indica a probabilidade em que uma propriedade passa a ter alta probabilidade de ser satisfeita por  $G(n,p)$ quando $n$ é suficientemente grande.

Uma variação do problema tipo Ramsey é a chamada \textit{propriedade anti-Ramsey} em que buscamos cópias \textit{rainbow} de um grafo $F$ em colorações próprias de arestas de $G(n,p)$, isto é, cópias de $F$ cujas arestas não repetem cor.
Denotamos essa propriedade por $G(n,p)\rbarrow F$.
Para cópias rainbow de $F$ em um grafo determinístico, o trabalho  de Fujita, Magnant e Ozeki~\cite{fujita2010rainbow} sumariza os resultados nesse caso. 
...
Os melhores limitantes sobre $er(t)$ foram obtidos por Lefmann e R{\"o}dl~\cite{lefmann1995erdHos}, que provaram

$$2^{c_1t^2}\leq er(t) \leq 2^{c_2t^2\log t},$$ 

em que $c_1$ e $c_2$ são constantes.

Podemos generalizar a coloração descrita em (\ref{it:canrain})-(\ref{it:cancan}) para encontrar outros grafos $H$ além de $K_t$, esta coloração é comumente chamada de \emph{canônica} \cite{lefmann1993canonical, axenovich2005canonical}.
...
Outro resultado nessa linha, para ciclos pares foi obtido por Alvarado, Kohayakawa, Morris e Mota \cite{alvarado2023canonical}.
Note que em uma coloração própria das arestas de um grafo $G$, toda cópia canônica de $H$ em $G$ é uma cópia rainbow de $H$.


O principal resultado desta tese trata da propriedade anti-Ramsey, em que buscamos obter colorações próprias das arestas de $G(n,p)$ que evitem cópias rainbow do grafo bipartido completo.
...
Para estimar o limiar para a propriedade anti-Ramsey 
%se baseia numa técnica introduzida por  Nenadov, Person, Škorić e Steger~\cite{NePeSkSt14}.
reduzimos o problema de grafos aleatórios para um problema determinístico em que analisamos como as cópias do grafo bipartido completo podem ocorrer em grafos com densidade limitada utilizando resultados de \cite{behague2024thresholds}, que estenderam o framework desenvolvido em \cite{NePeSkSt14}. Este último grafo de densidade limitada é que deve ser cuidadosamente colorido com relação às cópias rainbow de um bipartido completo. 
A estratégia utilizada aqui foi inspirada para provar o limiar de ciclos \cite{barros2021anti, NePeSkSt14} e grafos completos \cite{kohayakawa2019anti, NePeSkSt14}.
Porém, em nossa demonstração foi necessário tratar grafos bipartidos completos, que não são necessariamente simétricos, nem regulares.  

Um outro problema investigado durante a pesquisa foi introduzido por Conlon e Tyomkyn em \cite{conlontyomkyn}
e consiste de, dados os inteiros positivos $k$ e $n$, e um grafo $H$,  determinar o menor números de cores, escrito como $f_k(n,H)$, 
...
%%% Os trabalhos desses autores, que muitas vezes consideram grafos aleatórios, mostram que ainda existe muito a ser feito no desenvolvimento de uma teoria sólida para entendermos a existência de uma certa ordem em estruturas aparentemente caóticas, o cerne da Teoria de Ramsey.
 ...
 O Capítulo \ref{cap:anti} traz as condições para que a propriedade anti-Ramsey ocorra no grafo aleatório $G(n,p)$ com relação a conter cópias isomorfas ao grafo bipartido completo $\K$. 
A saber, dados inteiros $\ell,r \geq 3$, provamos que a função limiar para a propriedade do grafo aleatório colorido propriamente conter uma cópia rainbow de $\K$ é dada por
$n^{-1/m_2(\K)} = n^{(\ell+r-2)/(\ell r-1)}$ (vide Teoremas \ref{teo:conditional} e \ref{teo:mainrb}).
...
se e somente se, 
%para todo $\epsilon > 0$, existe $n_0$ tal que, se $n \geq n_0$, então $f(n) < g(n)$.
$\lim_{n \rightarrow \infty} \frac{f(n)}{g(n)} = 0$.

Um \textit{grafo} $G$ é um par de conjuntos $(V,E)$, também denotados por $V(G)$ e $E(G)$, os quais são
os conjuntos dos \textit{vértices} e das \textit{arestas} de $G$, respectivamente, em que $E(G)$ é o conjunto de pares de vértices.
Dizemos que um grafo $H$ é um \textit{subgrafo} de $G$
se $V(H) \subseteq V(G)$ e $E(H) \subseteq E(G)$. 
Por vezes, dados grafos $G$ e $H$, se existe um subgrafo $F\subseteq G$ que é isomorfo a $H$, dizemos que $F$ é uma \textit{cópia} de $H$ em $G$. 
...
aresta de $E(G)$ possui extremidades em $A$ e $B$, então dizemos que $G$ é um grafo \textit{bipartido}. 
...
Dado um grafo $G$, uma \textit{coloração de arestas} de $G$ é uma função $\chi: E(G) \rightarrow \mathbb{N}$. 
A coloração $\chi$ de $G$ é dita \textit{própria} se arestas adjacentes possuem cores distintas, i.e., para cada vértice $v \in V(G)$, tem-se $\chi(\{v,u\}) \neq \chi(\{v,w\})$ para cada par $u,w$ de vizinhos de $v$.
...
\begin{teorema}[\cite{bollobas1981threshold}]\label{teo:gnpcontem}
    Seja $H$ um grafo fixo. Então, $\hat{p} = n^{1/m(H)}$ é o limiar para a propriedade de que $G(n,p)$ contém uma cópia de $H$.
\end{teorema}
...
\end{defi}
  ...
  \begin{center}
			...
			\captionof{figure}{Uma coloração própria do $K_{6}$ com 5 cores identificando triângulos repetidos.} 
			\label{fig:K6} 
	\end{center}
...
\end{proof}
 ...
 \begin{center}
			\includegraphics[scale=0.75, keepaspectratio=true]{images/K222-5c.pdf}
			\captionof{figure}{A única coloração própria do $K_{2,2,2}$ com 4 cores a menos de permutações de cores.} 
			\label{fig:K222-5c} 
	\end{center}
  \begin{center}
			\includegraphics[scale=0.75, keepaspectratio=true]{images/K222-6c.pdf}
...
Os resultados descritos neste capítulo foram desenvolvidos em conjunto com Guiherme Mota e Walner Mendonça.
...
Person, Škorić e Steger~\cite{NePeSkSt14} foi estendido,
%reduz problemas tipo Ramsey de limitante inferior em (hiper)grafos aleatórios para um problema determinístico em (hiper)grafos com densidade máxima limitada.
recentemente, por Behague, Hancock, Hyde, Letzter e Morrison \cite{behague2024thresholds} que ampliaram a prova do framework de \cite{NePeSkSt14} para mais grafos quando restrito à propriedade anti-Ramsey.
...
%\begin{lema}[\cite{barros2021anti}]\label{lema:rbciclos}
 %        Sejam $\ell \geq 5$ um inteiro e $G$ um grafo tais que          ${m(G) < (\ell-1)/(\ell-2) = m_2(C_\ell)}$.          Então, $G \not\xrightarrow[]{rb} C_\ell$. \end{lema}
    
A essência da prova do Teorema \ref{teo:cyclecomplete} para ciclos está em usar uma estratégia de coloração própria nas arestas dos ciclos $C_\ell$ contidos no grafo $G$, de modo que toda cópia de ciclo $C_\ell$ tenha duas arestas coloridas com a mesma cor.
%
...
Eles observaram que tal sequência, com argumentos de densidade, impõe certas configurações restritas de como os novos ciclos aparecem entre os subgrafos, assim ao verificarem a ocorrência dessas configurações detalham alguns casos de como colorir arestas dessa sequência de subgrafos para evitar $C_\ell$ rainbow.
   ...
   Para provar que o limiar de grafos completos do Teorema \ref{teo:cyclecomplete} de que $G(n,p) \rbarrow K_\ell$, com $\ell \geq 5$, seja igual a $\hat{p}_{K_\ell} = n^{-1/m_2(K_\ell)} = n^{- 2/(\ell+1)}$, 
   %% os autores provaram o seguinte lema.
   ...
   %%    \begin{lema}[\cite{kohayakawa2019anti}]\label{lema:rbcomplete}
    %%     Sejam $\ell \geq 5$ um inteiro e $G$ um grafo tais que   $m(G) < (\ell+1)/2 = m_2(K_\ell)$.          Então, $G \not\xrightarrow[]{rb} K_\ell$.     \end{lema}
        %Para demonstrar o Lema \ref{lema:rbcomplete},
    é suficiente mostrar que todo grafo $G$ com $m(G) < (\ell+1)/2$  admite colorações próprias que evitam $K_\ell$ rainbow. 
...
Partindo da informação do Teorema \ref{teo:limiar}, o grupo de Behague, Hancock, Hyde, Letzter e Morrison \cite{behague2024thresholds} verificou com uma análise sobre densidades e grau mínimo o seguinte resultado:
...
\end{lema}
 ...
 O mesmo grupo de autores apoiados no Lema \ref{lema:rbgrau} e no Teorema \ref{teo:limiar} obtiveram o limiar da propriedade anti-Ramsey para uma subfamília de grafos regulares.
...
menos $4d$ vértices. 
 ...
 Note que como para $r \geq \ell \geq 1$ (note que $\delta(\K)=\ell$), temos

$$
    m_2(\K) = \frac{\ell r -1}{\ell+r-2} \geq \frac{\ell(\ell+1)}{2\ell+1},
$$
e disso vemos que o Lema \ref{lema:rbgrau} (e o Corolário \ref{cor:rbgrau}) não se aplica a grafos bipartidos completos.
...
\section{Limiar para grafos bipartidos completos}\label{sec:limiar}
    ...
    nós observamos que $K_{2,3r-2} \rb K_{2,r}$, 
    ...
    $K_{2,3r-2}\subset G(n,p)$ é $n^{-1/m(K_{2,3r-2})}$ e sabendo  que
    $m(K_{2,3r-2}) = (6r-4)/3r < (\ell r - 1)/(\ell+r-2) =
    m_2(K_{\ell,r})$, 
    temos que a presença de $K_{2,3r-2}$ no $G(n,p)$ força um limitante superior menor do que
     $n^{-1/m_2(K_{2,r})}$, i.e., nós concluímos o seguinte resultado:
    ...
    %%yyyyy
    Uma vez que $\K$ é um grafo estritamente 2-balanceado, tal como fora utilizado em ciclos e grafos completos, podemos pela aplicação do Teorema \ref{teo:limiar}, deduzir o Teorema \ref{teo:mainrb} a partir do seguinte resultado: %%que é provado na Seção ZZ.
    ...
    Nosso objetivo é encontrar em qualquer grafo $G$ com 
$m(G) < m_2(\K)$ uma coloração própria das arestas de $G$ em que toda cópia de $\K$ em $G$ contém duas arestas com a mesma cor.
...
%%% existe simetria entre l e r aqui? provavel que nao importe
  ...
  \begin{figure}[htb] \centering \includegraphics[]{images/interP1.pdf}
...
Em outras palavras, o Fato \ref{fato:ciclos} afirma que cada bipartido $\K$ que surge em $H_i$ dentro da sequência de subgrafos $(H_1, \ldots, H_t)$ é tal que ou ele não intersecta arestas de termos anteriores a $H_{i-1}$ ou ele está contido em uma intersecção múltipla com a forma de $K_{\ell+s, r+w}$. 
  ...
  \begin{figure}[htb] \centering \includegraphics[width=0.94\textwidth]{images/configA.pdf}
...
%%multiple figs:  https://tex.stackexchange.com/questions/119905/insert-multiple-figures-in-latex
 ...
 \begin{figure}[htb] \centering \includegraphics[width=0.95\textwidth]{images/configB.pdf}
   \caption{Configuração do tipo $B$.}
   \label{fig:configB}
 \end{figure}

   \begin{figure}[htb] \centering \includegraphics[width=0.95\textwidth]{images/configC.pdf}
...
Nosso resultado principal é semelhante ao que foi feito em ciclos e grafos completos no Teorema \ref{teo:cyclecomplete}
%Lemas \ref{lema:rbciclos} e \ref{lema:rbcomplete},
pois exibe colorações sem determinadas cópias rainbow. 
...
são menos presentes por conta da densidade elevada que possuem, e, aparecem no máximo duas vezes seguidas. 
 ...
 \begin{figure}[htb] \centering \includegraphics[width=0.95\textwidth]{images/tipoAcolor.pdf}
 ...
 \begin{figure}[htb] \centering \includegraphics[width=0.96\textwidth]{images/tipoBcolor.pdf}
...
e verificamos isso para grafos bipartidos completos.
...
A temática de coloração tipo anti-Ramsey vem se mostrando frutífera em  problemas e conjecturas, 
...
Sobre o segundo problema investigado, 
%enfatizamos que este trabalho fez uso importante dos resultados de %Nenadov, Person, Škorić e Steger~\cite{NePeSkSt14} e \cite{behague2024thresholds}. Auxiliados por estes trabalhos, 
foi o de estimar o limiar da propriedade anti-Ramsey de grafos bipartidos completos no grafo aleatório. 
Para isso, foi feita em uma análise determinística de grafos com densidades limitadas. 
Acreditamos que o uso desse procedimento possa ser replicado para mais classes de grafos. 
